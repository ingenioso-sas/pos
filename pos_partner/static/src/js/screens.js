...
//    var _t = core._t;
    ...
    var ClientListScreenWidgetExtend = screens.ClientListScreenWidget.include({
        line_select: function(event,$line,id) {
            try{
                var partner = this.pos.db.get_partner_by_id(id);
                let datebirt = new Date(partner.fecha_nac)
                let datenow = new Date()
                // verificar si es el cumpleaños
                if (datebirt.getUTCDate() == datenow.getUTCDate() && datebirt.getUTCMonth() == datenow.getUTCMonth()) {
                    this.confetti_start();
                }
            }catch(err){
                console.log(err)
        ...
        confetti_init: function() {
            ...
            function resetParticle(particle, width, height) {
                particle.color = colors[(Math.random() * colors.length) | 0] + (self.confetti.alpha + ")");
                particle.color2 = colors[(Math.random() * colors.length) | 0] + (self.confetti.alpha + ")");
            ...
            function runAnimation() {
                if (pause)
                    ...
                    if (!supportsAnimationFrame || delta > self.confetti.frameInterval) {
                        context.clearRect(0, 0, window.innerWidth, window.innerHeight);
                        updateParticles();
            ...
            function startConfetti(timeout, min, max) {
                ...
                window.requestAnimationFrame = (function() {
                    return window.requestAnimationFrame ||
                        ...
                        function (callback) {
                            return window.setTimeout(callback, self.confetti.frameInterval);
                        };
                })();
            ...
            function updateParticles() {
                ...
                for (var i = 0; i < particles.length; i++) {
                    ...
                    if (!streamingConfetti && particle.y < -15)
                        ...
                        particle.y += (Math.cos(waveAngle) + particle.diameter + self.confetti.speed) * 0.5;
                        particle.tilt = Math.sin(particle.tiltAngle) * 15;
                    }
                    if (particle.x > width + 20 || particle.x < -20 || particle.y > height) {
                        if (streamingConfetti && particles.length <= self.confetti.maxCount)
